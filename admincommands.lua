	...
	mainplr.Chatted:Connect(function(msg) --connecting a function to player's chat message, and naming the message "msg"
		...
		if string.sub(msg,1,1) == pref then-- if have prefix on first letter
			...
			elseif command[1] == ".headsize" then
				local plr = command[2]
				local size = command[3]
				if game.Players:FindFirstChild(plr) then
					local head = game.Players:FindFirstChild(plr).Character:WaitForChild("Head")
			...
			elseif command[1] == ".invis"  then
				local plr = command[2]
				if game.Players:FindFirstChild(plr) then
					local chr = game.Players:FindFirstChild(plr).Character
			...
			elseif command[1] == ".disco" then
				local plr = command[2] 
				if game.Players:FindFirstChild(plr) then
					local chr = workspace:FindFirstChild(plr)
			...
			elseif command[1] == ".movevals" then
				...
				local jump = command[4]
				if game.Players:FindFirstChild(plr) then
					local chr = game.Players:FindFirstChild(plr).Character
					local hum = chr.Humanoid	
					hum.WalkSpeed,hum.JumpPower = speed,jump --it jut changes the walkspeed var on humanoid
				end
